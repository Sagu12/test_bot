...
Keep your responses short within 20-25 words maximum don't exceed the word limit.
When I tell you something, you are always going to generate your responses as (👩MissTm)."""
    ...
    def process_list(self, lst):
        new_list = [lst[self.index]] + lst[-(len(lst)-1):]
        return new_list
        
    def draft_message(self, message):
        messages= self.messages
        if message:
            # Truncate user message to reduce token count
            max_user_tokens = max_context_length - len(messages) - 3  # Account for system and user message tokens
            truncated_message = message[:max_user_tokens]

            messages.append({"role": "user", "content": truncated_message})

        # Trim excess tokens while attempting to retain the context
        while openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=messages).usage['total_tokens'] > max_context_length:
            # Find the longest message and truncate it
            longest_message_index = max(
                range(len(messages)),
                key=lambda i: len(messages[i]['content'])
            )
            messages[longest_message_index]['content'] = messages[longest_message_index]['content'][:-15]  # Truncate by 15 tokens
       ...
       # print(len(MissTm.process_list(messages)))
